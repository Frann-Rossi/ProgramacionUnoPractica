...
/**
 * @brief Agrega un dato a la Pila. Si se supera el tope no lo agrega
...
int desapilar(Pila* p) {
    ...
    if (p->postope > 0){
        desapilado= p->valores[(p->postope)-1];
        (p->postope)--;
    }
    else{
        printf("\nLa pila esta vacia\n");
...
/**
 * @brief Retorna el valor en la cima de la Pila sin alterarla. Si la pila está vacía, el valor retornado no será significativo.
 * @param p la referencia de una Pila.
 * @return el último valor en ser agregado a la pila.
 */
int tope(Pila* p) {
    int tope = -1;

    if (p->postope > 0){
        tope= p->valores[(p->postope)-1];
    }
    else{
        printf("\nLa pila esta vacia\n");
    }
